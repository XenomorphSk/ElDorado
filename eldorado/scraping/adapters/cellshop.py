...
def fetch_cellshop_list(category_url_tmpl: str, page: int, timeout=30) -> str:
...
def parse_cellshop(html: str):
    soup = BeautifulSoup(html, "lxml")
    # tenta vários seletores comuns
    cards = (soup.select(".product-card, .ProductCard, .card-product") or
             soup.select("article.product, .product"))
    for c in cards:
        name_el = c.select_one(".product-title, .name, h3, h2")
        price_el = c.select_one(".price-final, .price, .current-price")
